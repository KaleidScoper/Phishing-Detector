...
# 修复：使用 map_location 加载模型
model = torch.load("automatic/model_storage/model-cc1.pkl", map_location=device, weights_only=False)  # 自动适配CPU/GPU
model.eval()
...
pre = TP / (TP + FP) if (TP + FP) > 0 else 0  # 避免除零错误
rec = TP / (TP + FN) if (TP + FN) > 0 else 0  # 避免除零错误
f1s = (2 * pre * rec) / (pre + rec) if (pre + rec) > 0 else 0  # 避免除零错误
